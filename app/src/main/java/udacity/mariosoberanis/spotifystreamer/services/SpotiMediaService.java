...
public class SpotiMediaService extends Service {
    ...
    */
    public static final String TRACK_STOP_BROADCAST_FILTER = "spoti-media-service-on-complete";
    public static final String TRACK_START_BROADCAST_FILTER = "spoti-media-service-track-started";
    ...
    /*
    * This will be true so long as the TrackPlayingActivity is active.  When active, we'll keep
    ...
    * Once the TrackPlayingActivity exits, this will be set to false, and we'll cancel the
    * notification when the current track finishes.
    */
    private boolean continueOnCompletion;

    private final IBinder mBinder = new SpotiMediaServiceBinder();

    public class SpotiMediaServiceBinder extends Binder {
        public SpotiMediaService getService() {
            return SpotiMediaService.this;
        }
    }
        ...
        try {
            ...
            mMediaPlayer.setOnPreparedListener(new MediaPlayer.OnPreparedListener() {
                ...
                public void onPrepared(MediaPlayer mp) {
                    ...
                    PreferenceManager.getDefaultSharedPreferences(getApplicationContext())
                            .edit()
                            .putString(SearchActivity.PREF_CURRENT_ALBUM, playListItem.getAlbumName())
                            .putString(SearchActivity.PREF_CURRENT_ARTIST_NAME, playListItem.getArtistName())
                            .putString(SearchActivity.PREF_CURRENT_ARTIST_SPOTIFY_ID, playListItem.getArtistId())
                            .putString(SearchActivity.PREF_CURRENT_TRACK_NAME, playListItem.getTrackName())
                            .putString(SearchActivity.PREF_CURRENT_TRACK_SPOTIFY_ID, playListItem.getTrackId())
                            .putString(SearchActivity.PREF_CURRENT_TRACK_URL, playListItem.getTrackUri())
                            .putBoolean(SearchActivity.PREF_IS_PLAYING, true)
                            .commit();

                    Intent intent = new Intent(TRACK_START_BROADCAST_FILTER);
                    intent.putExtra(SpotiStreamerActivity.KEY_CURRENT_TRACK, playListItem);
                    LocalBroadcastManager.getInstance(SpotiMediaService.this).sendBroadcast(intent);
                }
            });
    ...
    public boolean pause() {
        ...
        try {
            ...
            new NotificationTarget(
                    ...
                    SpotiStreamerActivity.ACTION_PLAY,
    ...
    public boolean play() {
        ...
        try {
            ...
            new NotificationTarget(
                    ...
                    SpotiStreamerActivity.ACTION_PAUSE,
                    mCurrentlyPlaying.getTrackName(),
                    mCurrentlyPlaying.getTrackId(),
                    mCurrentlyPlaying.getTrackImage(),
                    mCurrentlyPlaying.getArtistName(),
